...
- заказчик — «ПОСМ.РЦ» (не меняется);
- договор — доступен один в выпадающем списке (не меняется);
- поручение — формируется и заполняется системой на основании номера сделки, номера smart-процесса и индекса пользователя ERP-системы.

**Шаг 2. Конфигуратор заказа.**

Загрузить информацию об адресной программе, номенклатуре и услугах в заказе можно автоматически (режим «загрузка файла») или вручную («мануальный режим»).
...
- Загрузка файла
  ...
  {% cut "Добавление номенклатуры" %}
   ...
   Чтобы добавить номенклатуру необходимо указать:
    ...
    - характеристики (длина, ширина, высота, объем, вес, занимаемая площадь, количество неа палете).
...
**Шаг 5. Даты начала и завершения заказа и назначение исполнителя**
...
После команды «Закрыть» менеджер, назначенный ответственным по данному заказу, увидит новую заявку в списке мероприятий, а информация о новой заявке отправится в ERP.
